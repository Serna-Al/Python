...
elif (temp >= 20 and temp <= 25) or (viento >= 5 and viento <= 15):  # Corrección aquí
    print('Clima fresco')
...
if tiene_permiso and edad >= 21:
    print('Puedes conducir y ademas eres mayor de edad')
elif tiene_permiso and edad < 21:
    print('Puedes conducir pero aun eres menor de edad')
...
for indice, precio in enumerate(precios):
    if precio < 20:
        ...
        print(f'El indice del producto es {indice} y tiene el precio de {precio*0.9}')
    elif precio > 30:
        print(f'El indice del producto es {indice} y tiene el precio de {precio-(precio*descuento)}')
"""
...
while indice <= len(nombres) - 1:
    if nombres[indice] == amado_buscado:
        ...
        indice = indice + 1
        if encontrado == False: print('Amado no encontrado')
"""
...
while indice <= len(nombres) - 1:
    if nombres[indice] == amado_buscado:
        encontrado = True
        print(f'Amado encontrado en la posición {indice}')
        break  # Usamos break para salir del bucle una vez que se encuentra el amado
    indice += 1

# Verificamos si se encontró al amado
if not encontrado:
    print('Amado no encontrado')