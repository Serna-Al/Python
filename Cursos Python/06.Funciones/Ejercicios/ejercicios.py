...
"""
def calcular_impuestos(ingresos : int, tasa_impuesto : float) -> float:
    impuesto_debido = ingresos * tasa_impuesto
    return impuesto_debido

ingresos = 75000
tasa_impuesto = 0.3
impuesto_a_pagar =calcular_impuestos(ingresos,tasa_impuesto)
print(f"El impuesto a pagar es de {tasa_impuesto} y la cantidad es {impuesto_a_pagar}")
"""
...
def calcular_costo_envio(origen : str, destino : str) -> int:
    costo_envio : dict = {"Nueva York": {"Los Ángeles": 20, "Chicago": 10, "Miami": 15}, "Los Ángeles": {"Nueva York": 25, "Chicago": 17, "Miami": 20}}
    info_destinos : dict = costo_envio[origen] 
    costo_envio : int = info_destinos.get(destino,f"desde {origen} hasta {destino} no hay una ruta de envio todavia")
    return costo_envio
...
def convertir_moneda(cantidad_dolares : int, moneda_destino : str = "euros") -> float:
    tasas_cambio : dict = {"euros": 0.91, 'yenes': 106.23, 'libras': 0.77, "dolares" : 1.00}
    cantidad_destino = cantidad_dolares * tasas_cambio[moneda_destino]
    return cantidad_destino

dolares = 1000
yenes = convertir_moneda(dolares, "yenes")
euros = convertir_moneda(dolares)
print(yenes)
print(euros)
...
def aplicar_descuento(producto : str, moneda : str = "dolares", **kwargs) -> float:
    if kwargs and producto in kwargs.keys():
        precio = kwargs[producto]["precio"]
        descuento = kwargs[producto]["descuento"]
        precio_con_cambio = convertir_moneda(precio,moneda)
        precio_con_descuento = precio_con_cambio - precio_con_cambio * descuento
        return precio_con_descuento
    return -1

precio_final = aplicar_descuento("zapatos", "yenes", zapatos={"precio": 12.99, "descuento": 0.1}, collar={"precio": 9.99, "descuento": 0.2}, pulsera={"precio": 19.99, "descuento": 0.15}, gafas={"precio": 29.99, "descuento": 0.25})
print(precio_final)